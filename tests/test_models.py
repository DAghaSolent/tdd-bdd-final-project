...
class TestProductModel(unittest.TestCase):
    ...
    def test_update_a_product(self):
        ...
        product.description = "Sale Item"
        original_id = product.id
        product.update()
        self.assertEqual(product.id, original_id)
        self.assertEqual(product.description, "Sale Item")

        # Fetching all products back
        products = Product.all()

        # Asserting that the id hasn't changed and there is only one item
        self.assertEqual(len(products), 1)
    ...
    def test_delete_a_product(self):
        ...
        # Asserting that the product length is 1 symbolizing the product has been created sucessfully
    ...
    def test_list_all_products(self):
        ...
        # Asserting that the list is empty
        self.assertEqual(products, [])

        # Looping through 5 times to create 5 products
        for n in range(5):
            product = ProductFactory()
            product.create()

        # Fetching that the system can retrieve and list all the products
        products = Product.all()
    ...
    def test_find_product_by_category(self):
        ...
        # Using a loop to iterate to create products and save them to the database
        for product in products:
            product.create()
        ...
        # Retrieving the product by category
        found = Product.find_by_category(category)

        # Assert the count of products that match a specific category
        self.assertEqual(found.count(), count)

        # Looping through the found products and asserting that the categories match for the products
        for product in found:
